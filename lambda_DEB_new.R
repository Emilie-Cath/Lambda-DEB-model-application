...
lambda_bis <- function(L,f,Sm,TC,p_am,v,k,p_m,Eg,p_T,M_V,mu_V,k_L){
  lambdaE=(((v*Sm*Eg+p_T)*L^-1+p_m)/(k*f*p_am/v+Eg))*TC
  lambdaV=k*f*p_am*TC/(M_V*mu_V*v)*(((v*Sm*Eg+p_T)*L^-1+p_m)/(k*f*p_am/v+Eg))+(k_L-1)*(p_m+p_T*L^-1)*TC/(M_V*mu_V)
  lambda=lambdaE+lambdaV
  return(data.frame(L=c(L),lamE=c(lambdaE),lamV=c(lambdaV),lam=c(lambda)))
}

relashionship_bis <- function(num_points,f_val,species_names,data,TC,k_L){
  ...
  for (i in 1:length(species_names)){ #for every species
    ...
    for (k in 1:length(f_val)){ #for every f
      ...
      if (DEB_param$Model=="std"|DEB_param$Model=="stf"|DEB_param$Model=="stx"){ #for std, stf, stx
        lambda_value<- lambda_bis(L=size
                              ,f=f,Sm=DEB_param$s_M,TC=TC,p_am=DEB_param$p_Am,
                              v=DEB_param$v,k=DEB_param$kap,p_m=DEB_param$p_M,Eg=DEB_param$E_G
                              ,p_T=DEB_param$p_T,M_V=DEB_param$M_V,mu_V=DEB_param$mu_V,
                              k_L=k_L)
      }
      if (DEB_param$Model=="abj") { #for abj 
        ...
        for (j in 1:length(size)){
          if (size[j]<DEB_param$L_j){
            ...
            lambda_value=rbind(lambda_value,lambda_bis(L=size[j]
                                                   ...
                                                   M_V=DEB_param$M_V,mu_V=DEB_param$mu_V,k_L=k_L))}
...
pred_lam_half_life_bis <- function(f,obs,data,k_L){
  t_1_2_pred=NULL
  for (i in 1:nrow(obs)){
    # i=1
    name=obs$Species[i]
    ...
    pred_lam=lambda_bis(L=obs_line$L_corr,f=f,Sm=Sm,TC=obs_line$TC
...
#----OPENING DATA----
##DEB database##
data <- read.csv2("DEB_paramV8.csv",sep=",")
...
data_sub_bis=subset(data,data$Model=="asj")
every_species=unique(data_sub$Species)
rel_every_species= relashionship_bis(num_points=10,f_val=seq(0.5,1,by=0.1)
                                 ,species_names=every_species,data=data,k_L=k_L)
write.csv2(rel_every_species, file = "relationships_all_species_lambda_NEW_kL_0_5.csv", row.names = FALSE)
rel_every_species=read.csv2("relationships_all_species_lambda_NEW_kL_0_5.csv")
...
tiff("relathionship_all_NEW_kl0_5.tiff",  width =4 , height = 5, units = "in", res = 300)
plot(NULL,xlim=c(-10,8),ylim=c(-2,4), xlab="log10(weight)",ylab="log10(t1/2)",las=1)
...
# tiff("Legend_southampton.tiff",  width =4 , height = 5, units = "in", res = 300)
# par(xpd=TRUE, mar=c(8,4,4,3))
# plot(c(0,2),c(0,1),type = 'n', axes = F,xlab = '', ylab = '', main = '')
# legend("center",legend=c(" λ-DEB Predictions","Observations 1","Observations 2","Observations 3")
#        ,cex=1,col=c("grey","red","blue","green")
#        ,lty=c(1,NA,NA,NA),box.lty=0,pch=c(NA,16,16,16))
# dev.off()
# 

#####observations and predictions dataset 1  #####

rel_lam_ww= relashionship_bis(num_points=10,f_val=seq(0.5,1,by=0.1)
                          ,species_names=unique(obs$Species),data=data,k_L=k_L)
...
for (i in 1:length(species_names)){ #for each species
  ...
  title=paste0(name,"NEW_kL0_2.tiff")
  tiff(title, , width =4 , height = 5, units = "in", res = 300)
  plot(NULL,main=name,xlim=c(-8,6),ylim=c(-1,3), xlab="log10(weight)",ylab="log10(t1/2)",las=1)
...
rel_lam_ww= relashionship_bis(num_points=10,f_val=seq(0.5,1,by=0.1)
                          ,species_names=unique(review_new_bis$Species),data=data,k_L=k_L)
...
obs$t1_2_pred<- pred_lam_half_life_bis(f=f,obs=obs,data=data,k_L=k_L)



tiff("obs_vs_pred_DEB_OBS_f0.8_KL0.2_NEW.tiff",  width =4 , height = 5, units = "in", res = 300)
plot(NULL,xlim=c(0,3),ylim=c(-1,3),xlab="observed Half-life",
     ylab="λ-DEB predicted Half-life",las=1)
...
curve(0.2301+x*0.5531 , add = TRUE, col = "red")
curve(0.2301+x*0.5531-0.4631 , add = TRUE, col = "red4")
dev.off()

# 
# ##
#  k_L=0.8
# lam <- pred_lam_half_life(f=f,obs=obs,data=data)
# lam_bis <-  pred_lam_half_life_bis(f=f,obs=obs,data=data,k_L=k_L)
# 
# plot(log10(lam),log10(lam_bis))
# points(seq(-1,3,by=0.1),seq(-1,3,by=0.1),type="l")

# 
# ###pareil mais que sur les endotherms
# endos= subset(obs,obs$type=="endotherm")
# k_L=0
# endos_half_life_bis <- pred_lam_half_life_bis(f=f,obs=endos,data=data,k_L=k_L)
# endos_half_life <- pred_lam_half_life(f=f,obs=endos,data=data)
# plot(log10(endos_half_life),log10(endos_half_life_bis))
# points(seq(-1,3,by=0.1),seq(-1,3,by=0.1),type="l")
# 
# 
# ##que sur les ectotherms 
# ectos= subset(obs,obs$type=="ectotherm")
# k_L=0.9
# ectos_half_life_bis <- pred_lam_half_life_bis(f=f,obs=ectos,data=data,k_L=k_L)
# ectos_half_life <- pred_lam_half_life(f=f,obs=ectos,data=data)
# plot(log10(ectos_half_life),log10(ectos_half_life_bis))
# points(seq(-1,3,by=0.1),seq(-1,3,by=0.1),type="l")
# 
# ###sur le type de modèle 
# mod=subset(obs,obs$model=="abj")
# k_L=0.9
# mod$half_life_bis <- pred_lam_half_life_bis(f=f,obs=mod,data=data,k_L=k_L)
# mod$half_life <- pred_lam_half_life(f=f,obs=mod,data=data)
# plot(log10(mod$half_life),log10(mod$half_life_bis))
# points(seq(-1,3,by=0.1),seq(-1,3,by=0.1),type="l")
# 
# plot(NULL,xlim=c(0,3),ylim=c(-1,3),xlab="half-life old lam",
#      ylab="half life new lam",las=1)
# for (i in 1:nrow(mod)){
#   if (mod$tissue[i]=="whole body"& mod$type[i]=="endotherm" ){
#     color="red4"
#     if ( mod$element[i]=="C"){num=1}
#     if ( mod$element[i]=="N"){num=0}
#     if ( mod$element[i]=="S"){num=2}
#   }
#   if (mod$tissue[i]=="muscle"& mod$type[i]=="endotherm" ){
#     color="red"
#     if ( mod$element[i]=="C"){num=1}
#     if ( mod$element[i]=="N"){num=0}
#     if ( mod$element[i]=="S"){num=2}
#   }
#   if (mod$tissue[i]=="whole body"& mod$type[i]=="ectotherm" ){
#     color="red4"
#     if ( mod$element[i]=="C"){num=16}
#     if ( mod$element[i]=="N"){num=15}
#     if ( mod$element[i]=="S"){num=17}
#   }
#   if (mod$tissue[i]=="muscle" & mod$type[i]=="ectotherm"){
#     color="red"
#     if ( mod$element[i]=="C"){num=16}
#     if ( mod$element[i]=="N"){num=15}
#     if ( mod$element[i]=="S"){num=17}
#   }
#   points(log10(mod$half_life[i]),log10(mod$half_life_bis[i]),col=color,pch=num)
#   points(seq(-1,3,by=0.1),seq(-1,3,by=0.1),type="l")
# }
# ###checking the data
# name_endo=unique(endos$Species)
# DEB_endo=NULL
# for (i in 1:length(name_endo)){
#   name=name_endo[i]
#   DEB_endo=rbind(DEB_endo,subset(data,data$Species==name))
# }
# 
...
plot(log10(obs$t_1.2_.d.),log10(obs$t1_2_pred),xlim=c(0,3),ylim=c(0,3),xlab="log10(obst1/2)",
     ylab="log10(predt1/2)",pch=16,main="obs vs pred lambda DEB dataset 1")
points(seq(0,3,by=0.1),seq(0,3,by=0.1),type="l")
curve(0.3199+x*0.5282, add = TRUE, col = "red")
curve(0.3199+x*0.5282-0.5117 , add = TRUE, col = "red4")
...
plot(NULL,xlim=c(0,3),ylim=c(-1,3),xlab="observed Half-life",
     ylab="T&C predicted Half-life",las=1)
for (i in 1:nrow(obs)){
  if (obs$tissue[i]=="whole body"& obs$type[i]=="endotherm" ){
    color="red4"
    if ( obs$element[i]=="C"){num=1}
...
##prediction
review_new_bis$t_1.2_pred=pred_lam_half_life_bis(f=f,obs=review_new_bis,data=data,k_L=k_L)

#####Lambda-DEB predictions vs obs#####


tiff("obs_vs_pred_DEB_rev_KL0.2_NEW.tiff",  width =4 , height = 5, units = "in", res = 300)
plot(NULL,xlim=c(0,3),ylim=c(0,3),xlab="observed Half-life",
     ylab="λ-DEB predicted Half-life",pch=16,las=1)
for (i in 1:nrow(review_new_bis)){
...
Tac_rev=NULL
for (i in 1:nrow(review_new_bis)){
  if (review_new_bis$element[i]=="N")
  {Tac_rev=c(Tac_rev,TaC_t_1_2_N(Ww=review_new_bis$Mass..g.[i],t=review_new_bis$Body.temperature...C.[i]))}
  else {Tac_rev=c(Tac_rev,TaC_t_1_2_C(Ww=review_new_bis$Mass..g.[i],t=review_new_bis$Body.temperature...C.[i]))}
...
plot(log10(review_new_bis$t1.2),Tac_rev,xlim=c(0,3),ylim=c(0,3),xlab="log10(obst1/2)",
     ylab="log10(predt1/2)",pch=16,las=1)
points(seq(0,3,by=0.1),seq(0,3,by=0.1),type="l")
# dev.off()

tiff("obs_vs_pred_TaC_rev.tiff",  width =4 , height = 5, units = "in", res = 300)
plot(NULL,xlim=c(0,3),ylim=c(0,3),xlab="observed Half-life",
     ylab="T&C predicted Half-life",pch=16,las=1)
for (i in 1:nrow(review_new_bis)){
  if (review_new_bis$tissue[i]=="whole body"& review_new_bis$type[i]=="endotherm" ){
    color="blue4"
    if ( review_new_bis$element[i]=="C"){num=1}
...
#lambda estimation
lambda_H_d <- lambda_bis(L=size_Oncor,f=f,Sm=Sm_val
                     ,TC=TC,p_am=H_d_p$p_Am,v=H_d_p$v
                     ,k=H_d_p$kap,p_m=H_d_p$p_M
                     ,Eg=H_d_p$E_G,p_T=H_d_p$p_T,M_V=H_d_p$M_V
                     ,mu_V=H_d_p$mu_V,k_L=k_L)
...
tiff("ratio_ONCOR_O2_lambda_NEW.tiff",  width =4 , height = 5, units = "in", res = 300)
par(mar=c(4,4,3,5))
plot(lambda_H_d$Ww,ratio,xlab="Wet Weight (g)",ylab="ratio conso O /λ ",
     type="l",lwd=2,las=1)
dev.off()


##FIG 9B#### 
tiff("O2_lambda_Oncor_NEW.tiff",  width =4 , height = 5, units = "in", res = 300)
par(mar=c(4,4,3,5))  
plot(O2_conso$Ww,log10(O2_conso$O2/O2_conso$mat_quant)
     ,xlab="",ylab="",axes=F, type="l",col="black",lwd=2,las=1)
...
box() 
par(new=T) 
plot(O2_conso$Ww, log10(lambda_H_d$lam),
     pch=15,  xlab="", ylab="", axes=F, type="l", col="grey50",lwd=2,las=1)

# Legend axis Y on right
mtext("λ ",side=4,col="grey50",line=2.5)
axis(4, ylim=c(0,3),col="grey50",col.axis="grey50",las=1)
...
k_L=0.2
lambda_H_d_BIS <- lambda_bis(L=Size_Hd,f=f,Sm=Sm_val
                     ,TC=TC,p_am=H_d_p$p_Am,v=H_d_p$v
                     ,k=H_d_p$kap,p_m=H_d_p$p_M
                     ,Eg=H_d_p$E_G,p_T=H_d_p$p_T,M_V=H_d_p$M_V,
                     mu_V=H_d_p$mu_V,k_L = k_L)
lambda_H_d_BIS$Ww<- Ww(L=Size_Hd,f=f,p_am=H_d_p$p_Am
                   ,v=H_d_p$v,n_HE=H_d_p$n_HE
                   ,n_OE=H_d_p$n_OE,n_NE=H_d_p$n_NE
                   ,mu_E=H_d_p$mu_E)[[2]]
...
#      ,xlim=c(0,Max_Ww_at_f1),ylim=c(0,0.3),xlab="Wet Weight (g)"
#      ,ylab="λ ",type="l", lwd=2,las=1)
# points(lambda_H_d_BIS$Ww,lambda_H_d_BIS$lam,type="l",col="red", lwd=2)
# 
# 
# plot(lambda_H_d$lamE,lambda_H_d_BIS$lamE,xlab="lambdaE",ylab="lambdaE BIS")
# points(c(0,2),c(0,2),type="l")
# 
# plot(lambda_H_d$Ww,lambda_H_d$lamE
#      ,xlim=c(0,Max_Ww_at_f1),ylim=c(0,0.3),xlab="Wet Weight (g)"
#      ,ylab="λ ",type="l", lwd=2,las=1)
# points(lambda_H_d_BIS$Ww,lambda_H_d_BIS$lamE,type="l",col="red", lwd=2)
# 
# plot(lambda_H_d$Ww,lambda_H_d_BIS$lamE/lambda_H_d$lamE)
# 
# 
# plot(lambda_H_d$lamV,lambda_H_d_BIS$lamV,xlab="lambdaV",ylab="lambdaV BIS")
# points(c(0,2),c(0,2),type="l")


title=paste0("lambda_relation_On_f",f,"_TC_",temp,"NEW.tiff")
tiff(title,  width =4 , height = 5, units = "in", res = 300)
plot(lambda_H_d_BIS$Ww,lambda_H_d_BIS$lam
     ,xlim=c(0,Max_Ww_at_f1),ylim=c(0,0.3),xlab="Wet Weight (g)"
     ,ylab="λ ",type="l", lwd=2,las=1)
points(lambda_H_d_BIS$Ww,lambda_H_d_BIS$lamE,col="blue",type="l",lwd=2)
points(lambda_H_d_BIS$Ww,lambda_H_d_BIS$lamV,col="green",type="l",lwd=2)
dev.off()
...
k_L_effect=data.frame(Species=obs$Species,type=obs$type)

k_L_value=seq(0.1,1,by=0.1)
for (i in 1:length(k_L_value)) {
  kappa_L=k_L_value[i]
  a= pred_lam_half_life_bis(f=f,obs=obs,data=data,k_L=kappa_L)
  k_L_effect=cbind(k_L_effect,a)
}


## endotherms#### 
endos= subset(k_L_effect,k_L_effect$type=="endotherm")

df_graph=data.frame(kappa_L=c(),pref_half_life=c())
for (i in 1:(ncol(endos)-2)) {
  df_graph=rbind(df_graph, data.frame(rep(k_L_value[i],nrow(endos)),endos[[i+2]]))
}
colnames(df_graph)<-c("kappa_L","pred_half_life")

tiff("endotherm.tiff",  width =4 , height = 5, units = "in", res = 300)
boxplot(log10(pred_half_life)~kappa_L,data=df_graph,ylim=c(-0.5,2.5),xlab=""
        ,ylab="",las=1)
dev.off()
...
## ectothemrs#### 
ecto= subset(k_L_effect,k_L_effect$type=="ectotherm")

df_graph=data.frame(kappa_L=c(),pref_half_life=c())
for (i in 1:(ncol(ecto)-2)) {
  df_graph=rbind(df_graph, data.frame(rep(k_L_value[i],nrow(ecto)),ecto[[i+2]]))
}
colnames(df_graph)<-c("kappa_L","pred_half_life")

tiff("ectotherm.tiff",  width =4 , height = 5, units = "in", res = 300)
boxplot(log10(pred_half_life)~kappa_L,data=df_graph,ylim=c(-0.5,2.5),xlab=""
        ,ylab="",las=1)
dev.off()
